...
func loop(c *cli.Context) error {
	...
	log.Debug().Msgf("grpc dial to (%s) success with credentials (%s:%s)", c.String("server"), c.String("username"), c.String("password"))
		...
		go func(index int){
			...
			for {
				...
				if err := r.run(ctx); err != nil {
					log.Error().Msg("pipeline done with error")
					return
				}
			}
		}(i)
...
func (r *runner) run(ctx context.Context) error {
	...
	timeout := time.Hour // default building timeout
	...
	logger.Info().Msgf("Runner #(%d) ready to run.", r.index)
	...
	defaultLogger := pipeline.LogFunc(func(proc *backend.Step, rc multipart.Reader) error {
		loglogger := logger.With().Str("image", proc.Image).Str("stage", proc.Alias).Logger()
		...
		if serr := r.client.Upload(ctxmeta, work.ID, file); serr != nil {
			loglogger.Error().Err(err).Msg("log stream upload error")
		}
...
func extractRepositoryName(config *backend.Config) string {
	return config.Stages[0].Steps[0].Environment["REPO"]
}

// extract build number from the configuration
func extractBuildNumber(config *backend.Config) string {
	return config.Stages[0].Steps[0].Environment["BUILD_NUMBER"]
}
